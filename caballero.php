<?php
require_once './session.php'; // Se incluye el archivo de sesión para manejar la autenticación de usuarios.
...
try {
	...
	$sql = 'SELECT * FROM perfumes WHERE categoria = "caballero"'; // Se define la consulta SQL para obtener los perfumes de la categoría "caballero".
...
<head>
	...
	<link rel="stylesheet" href="./css/estilos-catalogo.css"> <!-- // Se incluyen los estilos CSS para el catálogo. -->
</head>
...
<body>
			...
			<nav class="navegacion">
				<ul class="menu">
					...
					<?php if (isLogged()) { ?> <!-- // Si el usuario ha iniciado sesión. -->
						...
						<?php if (isAdmin()) { ?> <!-- // Si el usuario es un administrador. -->
							<li class="menu-item">
	...
	<main id="contenido">
		<div class="contenido">
			<h2 class="titulo">Perfumes de Caballero</h2>
					...
					<?php foreach ($perfumes as $perfume) { ?> <!-- // Se itera sobre cada perfume obtenido. -->
						<div class="producto">
							...
							if (file_exists("./img/$img_path.jpg")) { // Si existe una imagen con extensión .jpg.
								...
								$perfume['imagen'] = "./img/default.webp"; // Se asigna la ruta de una imagen por defecto al perfume.
							}
							?>
